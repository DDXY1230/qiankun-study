  ...
  return scripts.map((script) => {
    ...
    if ((async || defer) && src && !module) {
      contentPromise = new Promise((resolve, reject) =>
        fiber
          ? requestIdleCallback(() =>
              fetchAssets(src, scriptCache, fetch, false, loadError).then(
                resolve,
                reject
              )
            )
...
export default function importHTML(params: {
  ...
  const htmlLoader = plugins
    ? compose(plugins.map((plugin) => plugin.htmlLoader))
    : defaultGetTemplate;
      ...
      return {
        ...
        getExternalScripts: () =>
          getExternalScripts(
            scripts
              .filter(
                (script) => !script.src || !isMatchUrl(script.src, jsExcludes)
              )
              .map((script) => ({
                ...script,
        ...
        getExternalStyleSheets: () =>
          getExternalStyleSheets(
            styles
              .filter(
                (style) => !style.src || !isMatchUrl(style.src, cssExcludes)
              )
              .map((style) => ({
                ...style,
  ...
  if (opts?.plugins.some((plugin) => plugin.htmlLoader)) {
    ...
    return (
      embedHTMLCache[url] ||
      (embedHTMLCache[url] = getHtmlParseResult(url, html, htmlLoader))
    );
  }
}
