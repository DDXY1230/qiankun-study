// 延迟函数
let delay = function delay(interval) {
...
(function () {
  ...
  upload_inp.addEventListener('change', function (e) {
    ...
    if (!file) return;
    // 限制文件大小的格式[方案一]  方案二:也可以在input标签中写 accept=".png,.jpg,.jpeg"
...
(function () {
  ...
  // 文件预览, 就是把文件对象转换成base64,赋值给图片的src属性即可
  upload_inp.addEventListener('change', async function () {
    let file = upload_inp.files[0],
      BASE64;
    if (!file) return;
  ...
  upload_inp.addEventListener('change', async function () {
    ...
    try {
      ...
      data = await instance.post('/upload_single', formData, {
        // 文件上传中的中间回调函数 xhr.upload.onprogress
        onUploadProgress(ev) {
          let {
            loaded,
            total
...
(function () {
  ...
  upload_inp.addEventListener('change', async function () {
    ...
    //我们重构集合的数据结构 给每一项设置一个值, 作为自定义属性存储到元素上,后期点击删除按钮的时候,我们基于这个自定义属性
...
(function () {
  ...
  // 拖拽获取dragenter dragleave dragover drop
