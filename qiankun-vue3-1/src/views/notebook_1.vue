...
<p>
  ...
  vue是异步渲染的框架,数据更新之后,dom是不会立刻渲染,$nextTick会在dom渲染之后被触发,用来
  获取最新的dom节点
  使用场景:
  1.在生命周期函数created中进行dom操作,一定要放到$nextTick函数中执行
  2.在数据变化后要执行某个操作,而这个操作需要使用随数据变化而变化的dom结构时,这个操作需要放到$nextTick函数中执行
</p>
  </div>
</template>
<script>
